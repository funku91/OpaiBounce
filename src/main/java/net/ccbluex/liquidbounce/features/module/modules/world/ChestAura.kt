...
object ChestAura : Module("ChestAura", Category.WORLD) {
    ...
    private val antiScaffold by boolean("NoScaffold", false)
    ...
    private var isScaffold:Boolean = moduleManager.getModule("Scaffold")?.state == true
    ...
    val onRotationUpdate = handler<RotationUpdateEvent> {
        if (isScaffold && antiScaffold) return@handler
    ...
    val onPacket = handler<PacketEvent> { event ->
        isScaffold=moduleManager.getModule("Scaffold")?.state == true
        if (isScaffold && antiScaffold) return@handler
