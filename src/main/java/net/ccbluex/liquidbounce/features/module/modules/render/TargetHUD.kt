...
object TargetHUD : Module("TargetHUD", Category.RENDER) {
    ...
    // Flux Settings
    private val fluxColorMode by choices("Opai-Color", arrayOf("Custom", "Health", "Rainbow"), "Health") { hudStyle == "Opai" }
    private val fluxColorRed by int("Opai-Red", 0, 0..255) { hudStyle == "Opai" && fluxColorMode == "Custom" }
    private val fluxColorGreen by int("Opai-Green", 120, 0..255) { hudStyle == "Opai" && fluxColorMode == "Custom" }
    private val fluxColorBlue by int("Opai-Blue", 255, 0..255) { hudStyle == "Opai" && fluxColorMode == "Custom" }
    private val fluxColorSpec by boolean("Opai-Gradient",true) {hudStyle == "Opai"}
    private val healthIcon by boolean("health Icon",false) { hudStyle == "Opai"}
    ...
    private fun renderMoon4HUD(x: Float, y: Float) {
        ...
        val nameLength = (Fonts.fontSemibold40.getStringWidth(boldName)).coerceAtLeast(
        ...
        // Text
        Fonts.fontSemibold40.drawStringWithShadow(boldName, 2F + 36F, 2F, -1)
    ...
    private fun renderFluxHUD(x: Float, y: Float) {
        ...
        RenderUtils.drawRoundedBorderRect(x+1, y+1, x + width - 1, y + height - 1, 1f, Color(40, 40, 40,245).rgb, Color.BLACK.rgb, 0F)
    ...
    private fun renderCompactHUD(x: Float, y: Float) {
        ...
        Fonts.fontSemibold35.drawCenteredString(text, x + width / 2, y + height / 2 - Fonts.fontSemibold35.fontHeight / 2 + 1, Color.WHITE.rgb, true)
    ...
    private fun renderNeonHUD(x: Float, y: Float) {
        ...
        // 名称文本
        Fonts.fontSemibold40.drawString(entity.name, 60F, 12F, Color.WHITE.rgb)
        ...
        Fonts.fontSemibold35.drawString(healthText, width - Fonts.fontSemibold35.getStringWidth(healthText) - 15F, barY + 2F, neonColor.brighter().rgb)

        // 距离显示
        val distance = mc.thePlayer.getDistanceToEntity(entity)
        val distanceText = "${decimalFormat.format(distance)}m"
        Fonts.fontSemibold35.drawString(distanceText, barX, barY + barHeight + 5F, Color(200, 200, 200).rgb)
