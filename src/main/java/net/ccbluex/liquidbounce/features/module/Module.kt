    ...
    var state = defaultState
        set(value) {
            ...
            if (!isStarting) {
                try {
                    if (value) {
                        JLayerUtils.playMP3("/assets/minecraft/opai/sounds/Enable/Sigma5.mp3")
                        WaterMark.showToggleNotification("Module Toggled", "$name has been §2Enabled§f!", true,930)
                    }else{
                        JLayerUtils.playMP3("/assets/minecraft/opai/sounds/Disable/Sigma5.mp3")
                        WaterMark.showToggleNotification("Module Toggled", "$name has been §4Disabled§f!", false,930)
                    }
                    val state = if (value) {
