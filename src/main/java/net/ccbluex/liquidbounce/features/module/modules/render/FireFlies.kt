...
// made by opZywl
object FireFlies : Module("FireFlies", Category.RENDER, gameDetecting = false) {

    init {
        state = false
    }

    private val ClientTheme by color("ClientTheme", java.awt.Color(0,0,0,120))
    ...
    private fun drawSparkPartsList(color: Int, firePart: FirePart, partialTicks: Float) {
        ...
        for (spark in firePart.sparkParts) {
            val c = applyOpacity(
                interpolateColor(java.awt.Color(-1), java.awt.Color(color), spark.timePC().toFloat()),
